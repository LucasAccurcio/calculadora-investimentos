...
const inputStyle = tva({
  base: 'border-background-200 flex-row overflow-hidden content-center data-[hover=true]:border-background-300 data-[focus=true]:border-primary-500 data-[focus=true]:hover:border-primary-500 data-[disabled=true]:opacity-40 data-[disabled=true]:hover:border-background-200 items-center',
...
const inputSlotStyle = tva({
  base: 'justify-center items-center web:disabled:cursor-not-allowed px-2',
});
...
const InputField = React.forwardRef<React.ComponentRef<typeof UIInput.Input>, IInputFieldProps>(
  function InputField({ className, ...props }, ref) {
    ...
    const sizeKey = parentSize ?? 'md';
    const fallbackFontSize = INPUT_FONT_SIZES[sizeKey] ?? 16;
    const baseFontSize =
      typeof flattenedStyle?.fontSize === 'number' ? flattenedStyle.fontSize : fallbackFontSize;
    const scaledFontSize = Number((baseFontSize * fontScale).toFixed(2));
